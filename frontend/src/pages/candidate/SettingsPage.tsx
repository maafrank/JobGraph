...
import { Button, Input, Select, Card, Modal, useToast } from '../../components/common';
...
export default function SettingsPage() {
  ...
  const [profileVisibility, setProfileVisibility] = useState<'public' | 'private' | 'anonymous'>('public');
  ...
  const handleSavePrivacySettings = async () => {
    ...
    try {
      await profileService.updateCandidateProfile({ profileVisibility });
      toast('Privacy settings saved successfully', 'success');
    } catch (error: any) {
      const message = error.response?.data?.error?.message || 'Failed to save privacy settings';
      toast(message, 'error');
    } finally {
    ...
    <Layout>
      ...
      {activeTab === 'privacy' && (
        <div className="space-y-6">
          <Card title="Profile Visibility">
            <div className="space-y-4">
              <Select
                ...
                options={[
                  { value: 'public', label: 'Public - Anyone can view my profile' },
                  { value: 'private', label: 'Private - Only employers I match with can see my profile' },
                  { value: 'anonymous', label: 'Anonymous - Employers see my skills but not my identity' },
                ]}
              />
              <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
                <p className="text-sm text-blue-800">
                  {profileVisibility === 'public' && 'Your full profile is visible to all employers on the platform.'}
                  {profileVisibility === 'private' && 'Only employers who have matched with you can view your profile details.'}
                  {profileVisibility === 'anonymous' && 'Employers can see your skills and qualifications but not your name or contact information until you accept a contact request.'}
                </p>
              </div>
              <Button
          ...
          <Card title="Resume Sharing">
            <div className="space-y-2">
              <p className="text-gray-600">
                Your resume is automatically shared with employers when:
              </p>
              <ul className="list-disc list-inside text-gray-600 space-y-1 ml-4">
                <li>You apply to their job posting</li>
                <li>They calculate matches and you're a qualified candidate</li>
              </ul>
              <p className="text-sm text-gray-500 mt-4">
                Resume shares expire after 90 days for security.
              </p>
            </div>
          </Card>
        </div>
        ...
        <Card title="Notification Preferences">
          <div className="bg-yellow-50 border border-yellow-200 rounded-md p-6 text-center">
            ...
            <p className="text-sm text-yellow-700 mt-2">
              Email and in-app notification settings will be available in the next release.
            </p>
          </div>
        </Card>
      )}
        ...
        <Card title="Danger Zone">
          <div className="border-2 border-red-300 rounded-md p-6 bg-red-50">
            ...
            <ul className="list-disc list-inside text-red-700 space-y-1 ml-4 mb-6">
              ...
              <li>All skill scores and interview history</li>
              <li>All job applications and matches</li>
              <li>Your uploaded resume and documents</li>
            </ul>
            <Button
              variant="danger"
